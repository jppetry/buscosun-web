 ...
 * (`src/router/placeSlugs.json`: [slug, name, lat, lon, land] je Ort), damit der
 * Punktforecast auf die statische Ortsseite verlinken kann (SEO/GEO 2026, E2)
 * und der Teilen-Link den Ort als Pfad-Slug tragen kann (Phase SH1: der Slug
 * loest Name, Koordinate UND Land auf, deshalb ist das Land seit 2026-09-08
 * die fuenfte Spalte).
...
const rows = PLACES.map((p) => [p.slug, p.name, +p.lat.toFixed(3), +p.lon.toFixed(3), p.country]);
