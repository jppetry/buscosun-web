...
Neu 2026-08-18: **WF0** (`audit/waldbrand-forecast.md`) diagnostiziert den Waldbrand-Forecast 0…+12 h
auf der Fusion — kein Code, Empfehlung Weg B gestuft, elf offene Fragen an Jan (§10), Start erst nach
Aufhebung des Composite-Vorbehalts (§2 a).**
...
- **Kein Test-Framework** (bewusst, s. `decisions.md`): stattdessen **51** Headless-Verifier `npm run verify:*` / `fusion:*` (gezählt 2026-08-18 inkl. `verify:fire-activity`; die Angabe „25" von 2026-08-03 galt vor der Waldbrand-Linie) (Node `--experimental-strip-types` importiert echte App-Module; teils gegen Live-Server → Netzabhängigkeit einkalkulieren). `npm run typecheck` muss grün sein.
