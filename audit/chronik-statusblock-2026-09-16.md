# Chronik des CLAUDE.md-Statusblocks bis 2026-09-16

> **Wortgleich ausgelagert am 2026-09-16 (abends)** aus `CLAUDE.md`, weil der Statusblock zur Chronik geworden war und die Verfassung nur den aktuellen Stand tragen soll („Historie gehört nicht in diese Datei"). Nichts ist gekürzt; die Absätze stehen in der Reihenfolge, in der sie in `CLAUDE.md` standen (jüngste Phase zuerst, ältere darunter). Die Belege je Linie liegen in den jeweiligen `audit/<thema>.md`. Für den aktuellen Stand gilt allein `CLAUDE.md`.

> **Stand: 2026-09-16 (abends).** **Aktuelle Phase: FI — Implementierung von buscosun Fusion auf dem Punkt-Cube, 0–336 h, Antwort < 2 s (`audit/fusion-implementierung.md`, Plan von Jan freigegeben 16.09.).** Jans Entscheidungen: Design-Doku = `ABLAUFPLAENE.md` + `QUELLENMATRIX.md`; **Integration** (Cube = Hauptmember der bestehenden Fusion, combine/dist/anchor bleiben); Zeitachse **109 native Schritte + markierte Interpolation**; Terrain geprüft: **kein Geländeprodukt in `buscosun-data`** (nur hmodel/urban), Gelände bleibt Laufzeit. Etappen AP0–AP12 (§4), Entscheidungen E-F-1…10 (§7.2). **AP0 umgesetzt (uncommitted):** Harness `npm run verify:pv-latency` (`scripts/verify-pv-latency.mjs`, Lab-Bündel per esbuild, CDP über `scripts/lib/cdpBrowser.mjs`; App-Bundle unberührt). **Gemessen (§9.0):** kalter jsDelivr-TTFB für 450-KB-Chunks p50 0,64 s / **p90 2,6 s**; `buscosun-data` **307 MiB** (doppelt über der nominellen 150-MB-Grenze, wird trotzdem ausgeliefert ⇒ kein Platz für Artefakte); Terrarium-Kacheln 100–150 KB, DEM-Box des Live-Pfads 0,7–1,0 s (= kritischer Pfad bei warmem CDN ⇒ Zwei-Skalen-DEM z8 + z11, kein Terrain-Produkt); **heutiger Cube-Leser im Browser 3,0 s p50 Desktop / 13,6 s Mobil-4G / 38,9 s 3G** (seriell, 25 RV-Frames à 63 KB), heutiges Produkt (Live-Pfad) 1,0 s / 1,9 s. ⚠ **V-FI-1: `point/<lauf>/run.json` ist am CDN veraltet** (Publisher purgt nur `index.json`, `run.json` wird je Stufe gemergt/beschnitten) ⇒ `readCubePoint` lieferte für t2/t3 **`null` ohne Fehler** — trifft auch den Archiv-Sammler. **Client-seitig behoben (§9.1, uncommitted):** Manifeste werden an `index.commit` gepinnt gelesen (`manifestStore`, `loadRunManifestFrom`, Herkunft `pinned|main|none`, V-PD-46-Wiederholung, `onSkip`-Grund statt stillem `null`); Sammler und CLI nutzen es; `verify:point-client` **70/70**; am lebenden Datum Zermatt/Wien t1 49 · t2 24 · t3 36 Schritte gepinnt. **Push von `main` vor 23:10 UTC schützt den heutigen Archiv-Slot** (der Workflow klont `main`). Publisher-Purge bleibt E-F-1. **Alle E-F-1…10 sind entschieden (§7.4, Jans Vertrauensvotum 16.09.):** Warm-up+Purge ja (AP12a nach AP1), Halo nein, progressives Laden Pflicht für 3G (kein Abnahmeprofil für p50, aber erste Darstellung < 5 s), `f_saison` = Nachtlänge `set`, kein Gazetteer, kein Terrain-Produkt (z8+z11), Massenaufrufer nach AP11, `latest.json` bedingt, **PA2 ja (AP-PA2 vor AP9)**, Rice bleibt. Werkzeugfalle: der Bash-Kanal schneidet bei ≈ 8 KB ab — große Dateien nur per Write. ✅ **AP1 umgesetzt (16.09. nachmittags, §9.2, uncommitted):** paralleler Leser `readPointBundle` (`src/point/client/readPoint.ts`) mit IndexedDB-Cache je Pfadregel (`cache.ts`), Worker-Pool (`decodePool.ts`/`decodeWorker.ts`), Zwei-Skalen-DEM z11 + z8 (`terrain.ts`), Chunk-Adresse aus dem Index (`cubeAddress`), `run.json@commit` nur für Provenienz, Radar-Frames nur auf Ausgabezeiten, Fetch-Prioritäten, Ausweichweg `raw.githubusercontent` mit 2,5-s-Hedge; CLI `point:read` fährt das Bündel. `verify:point-client` **112/112**. **Gemessen (160 + 32 Läufe):** Kern warm **189 → 168 ms** Desktop / **367 → 325 ms** Mobil-4G (Gate ≤ 400 / ≤ 1 000 **grün**); kalt-neu Kern 0,84–1,46 s Desktop, 2,2–2,3 s Mobil-4G (bytes-gebunden: 1,9 MB), 8,7 s 3G (AP12 Pflicht); erste Darstellung Desktop 0,8–1,3 s; Worker auf Mobil 367 gegen 820 ms Hauptthread. ⚠ **V-FI-5: jsDelivr antwortet vorübergehend 403 nach 1–8 s (auch statt 404; Genf-Chunk 15 min lang)** ⇒ kein Retry, Sonde = „nicht da", Hedge + raw-Ausweichweg; E-F-1-Warm-up muss 403 als Fehlschlag zählen. **V-FI-6:** statische Produkte gepinnt = MISS je Publish ⇒ `@main` + 12 h. Long Tasks in headless-shell nicht messbar (AP11 Real-Device). ✅ **AP-PA2 umgesetzt (16.09. abends, §9.3, uncommitted):** Punktliste 243 → **410** — DE 208 unverändert, **AT 84 (TAWES), CH 101 + LI 1 (SMN)**, 16 Nachbarn (CZ/SK/DK/NL/LU, von PA1 über den WMO-*Block* als AT/CH etikettiert) richtig benannt und im alten Live-Profil; Regel = Katalogstation am Ort einer Netzstation (gleiche Kennung ≤ 5 km oder ≤ 2 km, |Δz| ≤ 50 m — aus 171 Paaren gemessen), Kennung = Katalog, **Position/Höhe = Messstelle** (Säntis-DEM 1 925 → 2 370 m); Stationsprodukt über die Kennung; **`rr1h` = Stundensumme aus sechs 10-min-Werten** (die alte Momentaufnahme × 6 verfehlte 9 von 14 nassen POI-Stunden; SMN-Summe = POI auf die Stelle), `td`/`p` gefüllt; **V-FI-5 traf den Sammler** (38 × 403, 23 Punkte ohne Cube) ⇒ raw-Ausweichweg am selben Ref (`lib/rawFallback.mjs`). Lokaler Slot 410 Punkte, 17,5 MiB (43,7 KB/Punkt, +3 %), ≈ 6,7 GB/Jahr (E-U-13 dringender), 0 Fehler. `verify:punktarchiv` **87/87**. ✅ **AP12a umgesetzt (§9.4, uncommitted):** `scripts/point/cdnSync.mjs` — nach dem Landen: jede **geänderte** Datei purgen (`git diff --name-status`; run.json zuerst; in place geänderte `hmodel`-Chunks ausgenommen — gemessen 124 je t1-Job, V-PD-62), Index mit Commit-Prüfung, run.json Byte-Vergleich, dann alles Neue wärmen (Chrome-`Accept-Encoding`, **403/Frist = Fehlschlag mit Wiederholung**), Budget je Job (t1 240 · t2/t3 180 s, Regel F), Fristen je Abruf + harte Obergrenze (der lokale Publisher-Nachbau hing ohne sie); Schalter `POINT_CDN_SYNC=0` / `POINT_CDN_DRY=1`. Gemessen am echten t1-Job 19:40 UTC: A 209 · M 126 · D 266; Trockenlauf **210/210 ok, alle MISS, 0 × 403, 86 MiB in 28 s**. `verify:point-data` **969/969**, `verify:repack` 348/348, typecheck 0. **Jans Gate (MANUELLE-SCHRITTE §15): Push von `buscosun-web/main` (AP1+PA2+AP12a zusammen) vor 23:10 UTC**; Vorlagen-Kopie optional; Abnahme (b) `verify:pv-latency --only=bundle` nach dem ersten Job mit neuem Publisher (Basis: kalt Kern p50 1 458 ms). **Nächste Etappe: AP2** (`cubeSource.ts` + `pointSource: 'cube'`, S&F).
>
> **Davor (2026-09-16 vormittags): PD-U — Umsetzungsplan vor der Implementierung von buscosun Fusion (`audit/punktdaten-umsetzungsplan.md`), Bereitschaft §8.1 grün.** Lokal vorbereitet, nichts committet, nichts gepusht: das Punktarchiv PA1 (`scripts/punktarchiv/*`, 243 Punkte, Sammler beider Pfade, `verify:punktarchiv` 56/56, erster Slot lokal in `C:\dev\buscosun-archiv`), die Retention-Korrektur (`retainRuns` in `prune.mjs` — die Gesamtsicht löschte den vom Stufen-Boden behaltenen t3-Lauf, am Remote hielt t3 EINEN Lauf), der t2-Slot `30 4,10,16,22` in der Vorlage (MOSMIX-L kam am alten Slot 27 min zu früh), das Stadt-Raster `point/static/urban/v1/` (GHS-BUILT, BigTIFF+LZW, 111 KB), `declined` für MOSMIX-S/KENDA-CH1, Hash-Diff je `hmodel`-Spalte, eine Bereitstellungs-Messreihe (79 Proben). **Befund:** `audit/punktarchiv.md` und `PUNKTVORHERSAGE_0-336H.md` existieren nirgends — PA0 ist im Plan rekonstruiert. **Stand 2026-09-15:** `f13661c` gepusht (Jan), Archiv-Slot gepusht (Jan), `buscosun-data` `5ea830a` (t2-Slot + `point/static/urban/v1/`, mit Jans Freigabe). Archiv-Workflow am Remote (`0df940b`, Slot 23:10 UTC). **M-1 umgesetzt (Jan: „alle Punkte", 15.09.):** 925/850/700 auch in t3 (gemessen +3 MiB, gratis), IFS-ENS σ_ens + u10/v10 im 48-h-Raster mit **24 Windmembern** (Jans Wahl „mach die 24 member variante"; kalt 863 MiB je IFS-ENS-Lauf. ⚠ **Die Zahl, auf der die Wahl beruhte — „+530 MiB = +31 %" — war falsch:** sie verglich fünf gegen vier IFS-ENS-Stunden (der Remote gibt 144 h an ICON-EPS global). Aus dem `.index` gerechnet und am Bau bestätigt: 50 Windmember **+395 MiB = +23 %**, 24 Member **+190 MiB = +11 %**, beides unter der Schwelle; σ24/σ50 Median 8,0 %, p90 20,5 %, unverzerrt; Rückweg `POINT_ECMWF_WIND_MEMBERS: '50'` — §4.6a, Empfehlung 50), Member-Quantile q10/q90 in t2/t3 (`ensemble-members`, Kelvin-Versatz nur für Werte), `hModEff` je Schritt aus den tragenden Quellen inkl. abgeleiteter ECMWF-Höhe (`fillHModEff`, hinter dem Join), `axis.gaps`/`usableToH` im Index — Gates 947/947 · 63/63 · typecheck 0; **committet und gepusht (`717cc12`, Jan, 16.09. 05:40 UTC)**. **Bereitschaftsprüfung 2026-09-16, 09:00 UTC (Plan §8.1):** Datenseite grün — Archiv-Cron lief (Slot 15.09. 23:20 UTC, 243 Punkte, 0 Konflikte), t3 hält 2 Läufe, Stationen `ageH` 1,7 h, `urban/v1` am Remote, Lauf 65 (t1) baut mit dem neuen Code (`axis` im Index, `hModEff` je Schritt); t3/t2 folgen 09:55/10:30 UTC. Verifier 947/947 · 63/63 · 56/56, typecheck 0, Build 241/241, Budget unverändert. **Offen:** Cube-Adapter hinter Flag (U-20) und `calib.json`-Provenienzregel = erste Etappen der Implementierung; AROME-Ablösung vor 2026-11-01; Messreihe (1 Tag), t3-Slot; 24 vs. 50 Windmember (Empfehlung 50); V-PD-62 (`hmodel.changed` feuert bei jedem ECMWF-Laufwechsel, weil die abgeleitete Höhe am `sp` hängt). Davor (2026-09-14 vormittags): **D — die Beschreibungen im Daten-Repo aufgefrischt (`audit/datenrepo-beschreibungen.md`), nachdem Jan sie als veraltet gemeldet hat. Der README des Daten-Repos stammte von PD-A und beschrieb einen Betrieb, den es seit PD-B/Block F nicht mehr gibt; neu ist er an die Konstanten gebunden, damit er nicht wieder still verrottet. Dazu: `point/index.json` kündigt jetzt das statische Produkt an, das seit PD-E veröffentlicht und im Register unsichtbar war. Gate grün (point-data 912/912, point-client 63/63, Budget unverändert). **Die Kopie des READMEs ins Daten-Repo ist Jans Gate.****
>
