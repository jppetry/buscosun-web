...
- **Governor-Hebelordnung (least-visible-first):** FPS-Abbau (30→24→20) zuerst; als **letzter** Hebel — nur am FPS-Floor bei fortgesetztem Einbruch — fällt die **Trail-Auflösung** auf 0,5× (Phase P2). Trail-0,5× macht Partikel sichtbar weicher, daher ganz unten in der Leiter und bei Erholung zuerst zurückgenommen. Capable Geräte erreichen diese Sprosse nie. Die Partikel*zahl* bleibt in **keinem** Tier ein Hebel.
- **Repaint-Disziplin (Phase P3):** Der selbst-perpetuierende Wind-Repaint-Loop pausiert, wenn nichts sichtbar ist (`visibilitychange`→hidden, Offscreen via `IntersectionObserver`) — spart Akku/Hitze, damit die Zielrate über Zeit hält. Pause greift nur in nicht-sichtbaren Zuständen → sichtbar/aktiv byte-identisch, keine Regression.
